...
@section('page-subtitle', 'Ubah penilaian pegawai')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('assessments.update', $assessment) }}" method="POST">
                                ...
                                <select class="form-select @error('employee_id') is-invalid @enderror" 
                                    ...
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->id }}" 
                                                {{ old('employee_id', $assessment->employee_id) == $employee->id ? 'selected' : '' }}>
                    ...
                    @php
                        $currentScore = $assessment->scores->where('criteria_id', $criterion->id)->first();
                    @endphp
                    <div class="card mb-3">
                        <div class="card-body">
                                        ...
                                        <div class="col">
                                            <div class="form-check text-center">
                                                <input class="form-check-input @error('scores.' . $criterion->id) is-invalid @enderror" 
                                                       ...
                                                       value="{{ $i }}"
                                                       {{ old('scores.' . $criterion->id, $currentScore ? $currentScore->score : '') == $i ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" 
                                          ...
                                          placeholder="Tambahkan catatan atau komentar mengenai penilaian ini...">{{ old('notes', $assessment->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
